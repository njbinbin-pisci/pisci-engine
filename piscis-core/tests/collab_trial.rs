...
use piscis_core::heartbeat::{build_pool_heartbeat_message, collect_pool_attention, PoolAttention};
use piscis_core::models::{KoiTodo, PoolMessage, PoolSession};
...
use piscis_core::trial::effective_trial_koi_status;
use serde_json::json;
...
fn collab_trial_enrich_pool_message_metadata_records_structured_signal() {
    let metadata =
        enrich_pool_message_metadata(json!({}), "[ProjectStatus] ready_for_piscis_review @piscis");
    assert_eq!(
        metadata["coordination"]["signal"].as_str(),
        Some(STATUS_READY)
    );
    assert_eq!(
        metadata["coordination"]["mentions_piscis"].as_bool(),
        Some(true)
    );
    assert_eq!(metadata["mentions"]["piscis"].as_bool(), Some(true));
    assert_eq!(
        coordination_event_type_for_content("[ProjectStatus] ready_for_piscis_review"),
...
fn collab_trial_assessment_prefers_structured_coordination_metadata() {
    let messages = vec![sample_message(
        ...
        json!({
            "coordination": {
                "signal": CoordinationSignalKind::ReadyForPiscisReview.as_status_str(),
                "mentions_piscis": true,
            }
        }),
        Some("coordination_signal"),
    )];
    let assessment = assess_project_state(&messages, &[sample_todo("done")], &["koi-1".into()]);
    assert_eq!(assessment.decision, ProjectDecision::ReadyForPiscisReview);
    assert_eq!(assessment.explicit_piscis_handoff_count, 1);
}
...
fn collab_trial_assessment_keeps_legacy_text_signal_compatibility() {
    let messages = vec![sample_message(
        1,
        "koi-1",
        "[ProjectStatus] ready_for_piscis_review @piscis",
        json!({}),
        None,
    )];
    let assessment = assess_project_state(&messages, &[sample_todo("done")], &["koi-1".into()]);
...
fn collab_trial_all_done_without_handoff_requires_supervisor_decision() {
    ...
    assert!(assessment.summary.contains("Piscis must inspect the pool"));
}

#[test]
fn collab_trial_needs_review_without_handoff_still_requests_piscis_review() {
...
#[test]
fn collab_trial_collect_pool_attention_uses_shared_pool_piscis_session_id() {
    let pool = sample_pool();
    let messages = vec![sample_message(
        1,
        "koi-1",
        "@piscis [ProjectStatus] waiting",
        json!({}),
        None,
    )];
    ...
    let attention = collect_pool_attention(&pool, &messages, &todos, &koi_ids, 0)
        .expect("attention should be raised for explicit piscis mention");
    assert_eq!(attention.session_id, "piscis_pool_pool-1");
    assert!(attention.summary.contains("Recent attention events: 1"));
}

#[test]
fn collab_trial_collect_pool_attention_recognizes_delegated_piscis_mention() {
    use piscis_core::project_state::contains_piscis_mention;

    assert!(contains_piscis_mention("@!Piscis please review the board"));
    assert!(!contains_piscis_mention("@!Koi do something"));

    let pool = sample_pool();
    let messages = vec![sample_message(
        2,
        "user",
        "@!Piscis: unblock this project",
        json!({}),
        None,
    )];
    let attention = collect_pool_attention(&pool, &messages, &[], &[], 1)
        .expect("@!Piscis mention should flag pool attention");
    assert_eq!(attention.session_id, "piscis_pool_pool-1");

    let forced = piscis_core::heartbeat::build_forced_mention_attention(&pool, &messages, &[], &[])
...
fn collab_trial_build_pool_heartbeat_message_keeps_no_archive_instruction() {
    let attention = PoolAttention {
        ...
        session_id: "piscis_pool_pool-1".into(),
        summary: "Pool summary".into(),
        assessment: ProjectAssessment {
            decision: ProjectDecision::ReadyForPiscisReview,
            active_todo_count: 0,
            ...
            explicit_piscis_handoff_count: 1,
            integration_ready_count: 0,
            dependency_blocked_count: 0,
            attention_reasons: vec!["Project is ready for Piscis review".into()],
...
// that a heartbeat can act on, or (b) ReadyForPiscisReview for Piscis to close.
...
#[test]
fn convergence_task_failed_superseded_by_later_explicit_piscis_handoff_resolves() {
    // Failure happens first, then a later explicit @piscis handoff should let
    ...
    assert_eq!(
        assessment.decision,
        ProjectDecision::ReadyForPiscisReview,
        "explicit piscis handoff after a task_failed must re-open convergence"
    );
    assert_eq!(assessment.explicit_piscis_handoff_count, 1);
    assert_eq!(
        assessment.task_failed_count, 1,
        "the historical failure is still counted so Piscis can see it in the summary"
    );
}
...
fn convergence_same_sender_flip_flop_prefers_latest_signal() {
    // If a reviewer first signals ready_for_piscis_review then retracts with
    ...
    assert_eq!(assessment.follow_up_signal_count, 1);
    assert_eq!(assessment.explicit_piscis_handoff_count, 0);
...
#[test]
fn convergence_collect_pool_attention_flags_ready_without_explicit_piscis_handoff() {
    // "[ProjectStatus] ready_for_piscis_review" without `@piscis` is a stuck
    // state: no one explicitly handed the project to Piscis. Heartbeat must
    // still raise attention so Piscis inspects the pool.
    let base = Utc.with_ymd_and_hms(2026, 4, 17, 10, 0, 0).unwrap();
    let pool = sample_pool();
    let messages = vec![message_at(
        10,
        at_ms(base, 0),
        "koi-1",
        "[ProjectStatus] ready_for_piscis_review",
        json!({
            "coordination": {
                "signal": CoordinationSignalKind::ReadyForPiscisReview.as_status_str(),
                "mentions_piscis": false,
    ...
    assert!(
        ...
        "attention reason should name the missing @piscis handoff"
    );
}
...
fn convergence_all_done_without_handoff_raises_supervisor_decision_attention() {
    ...
    let attention = collect_pool_attention(&pool, &[], &todos, &[], 0)
        .expect("all-done-without-handoff should still require Piscis attention");
    assert_eq!(
        attention.assessment.decision,
        ProjectDecision::SupervisorDecisionRequired
    );
    assert!(
        attention
            ...
            .attention_reasons
            .iter()
            .any(|r| r.contains("Piscis must make the next global decision")),
...
fn convergence_collect_pool_attention_ignores_already_seen_events() {
    ...
    assert!(
        ...
        "cursor must not re-ingest the same @piscis mention as a fresh event"
    );
}
...
fn convergence_supervisor_required_stays_on_radar_after_cursor_advances() {
    ...
    // supervisor attention just because @piscis was mentioned earlier.
    let base = Utc.with_ymd_and_hms(2026, 4, 17, 10, 0, 0).unwrap();
    let pool = sample_pool();
    let messages = vec![message_at(
        7,
...
fn convergence_multiple_handoffs_across_senders_aggregate_cleanly() {
    // When several senders converge in parallel on ready_for_piscis_review
    // with @piscis handoff and no unfinished work remains, the assessment
    ...
    assert_eq!(assessment.decision, ProjectDecision::ReadyForPiscisReview);
    assert_eq!(assessment.explicit_piscis_handoff_count, 3);
