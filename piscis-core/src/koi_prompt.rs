...
- **Handoff messages must propagate the protocol, not just the task.** When you `@!mention` another agent to hand off work, your message MUST include three things, not one: (1) WHAT to do (the deliverable you expect), (2) WHERE the inputs are (file path, spec link, prior message reference), and (3) HOW to report completion \u{2014} name the expected next reporting target (return to you, hand to a third party identified from `org_spec`, or signal `@piscis`) and the `[ProjectStatus]` signal expected at completion. A handoff that says only \"do X\" silently transfers the cognitive load of figuring out completion semantics to the receiver, and receivers commonly drop the protocol when their attention is consumed by production. Treat your handoff message as the receiver's task brief.\n\
- Identify the next responsible party from project context, not from a fixed role catalogue. Inputs in priority order: (1) the project's `org_spec` (which agent owns this kind of work), (2) the latest task description in pool_chat, (3) the most recent @mention chain. If multiple inputs disagree, prefer org_spec. If no input identifies the next party with confidence, do NOT guess and do NOT default to any role name \u{2014} state the ambiguity in pool_chat and let Piscis route.\n\
...
- Status signals (place verbatim inside your pool_chat message so Piscis can reason about project state):\n\
  ...
  - `[ProjectStatus] ready_for_piscis_review` \u{2014} use ONLY after your own `complete_todo` has succeeded and your branch/result is ready for Piscis supervisor review and possible merge.\n\
- Never unilaterally declare the project complete. If you believe the project may be done, signal `@piscis`; do not poll peer agents for agreement.\n\
- Only Piscis or the user directly assigns work to you. Other agents use plain `@mention` for notification or `@!mention` for explicit delegation. The task board (pool_org) and chat (pool_chat) are your sources of truth; do not rely on heartbeat, trial, or other harnesses to repair missing coordination.\n"
}
...
- Your changes are auto-committed when the run ends; do NOT run `git add`, `git commit`, `git merge`, `git rebase`, or `git push` yourself \u{2014} branch integration is Piscis supervisor's responsibility. When your code work is done, note in pool_chat what changed, what verification passed, and that the branch is ready for Piscis review.\n\
...
- If your task depends on another Koi's code, set depends_on on the board (or ask Piscis to assign with depends_on) and wait until Piscis merges that branch before expecting downstream work to start.\n\
...
- No nested pool rule: do NOT initiate nested multi-agent collaboration from inside a Koi run. If the task grows into multi-domain, quality-sensitive work that needs separate implementation/review/QA coordination, finish or block your current scoped task and signal `@piscis` / `[ProjectStatus] follow_up_needed` so Piscis can coordinate at the pool level.\n\
...
\n\
3. **Continuation check.** If your output requires another specific agent to continue, confirm a `[ProjectStatus] follow_up_needed` post with that agent's line-start `@!mention` exists from THIS run. Identify the next responsible party from `org_spec` / task description / pool_chat history per the Coordination Protocol \u{2014} do NOT default to a role name. If your output looks like a project-ready conclusion, confirm `[ProjectStatus] ready_for_piscis_review @piscis` exists from THIS run AND was posted only after your `complete_todo` succeeded. Do NOT @mention peer agents for agreement.\n\
\n\
...
mod tests {
    ...
    /// signal (@piscis / ready_for_piscis_review), and the runtime safety
    ...
    fn capabilities_prompt_preserves_koi_routing_rules() {
        ...
        for required in [
            ...
            "signal `@piscis` / `[ProjectStatus] follow_up_needed`",
        ] {
            assert!(
