//! Shared CLI argument parsing for the `openpiscis-headless` binary.
//!
...
use piscis_core::host::{HeadlessCliMode, HeadlessCliRequest, HeadlessCliResponse};

/// Opinionated default for the usage banner printed on `--help` / bad args.
pub const USAGE: &str = concat!(
    "Usage:\n",
    "  openpiscis-headless                                     # interactive REPL (same as `chat`)\n",
    "  openpiscis-headless chat                                # interactive REPL\n",
    "  openpiscis-headless run --prompt <text> [--workspace <dir>] [--mode piscis|pool] [--output <file>]\n",
    "  openpiscis-headless run --input <request.json> [--output <result.json>]\n",
    "  openpiscis-headless capabilities [--mode piscis|pool]\n",
    "  openpiscis-headless version\n",
);
...
pub fn parse_mode(raw: &str) -> Result<HeadlessCliMode, String> {
    match raw.trim().to_ascii_lowercase().as_str() {
        "piscis" => Ok(HeadlessCliMode::Piscis),
        "pool" => Ok(HeadlessCliMode::Pool),
        other => Err(format!(
            "Unsupported mode '{other}'. Use 'piscis' or 'pool'."
        )),
    }
}
...
/// Parse the trailing portion of `openpiscis-headless run <...>` into a
...
/// Parse `openpiscis-headless capabilities <...>` into a mode selector.
pub fn parse_capabilities_mode(args: &[String]) -> Result<HeadlessCliMode, String> {
    let mut mode = HeadlessCliMode::Piscis;
    let mut i = 0usize;
    while i < args.len() {
        match args[i].as_str() {
            "--mode" => {
...
mod tests {
    ...
    fn parse_mode_round_trip() {
        assert_eq!(parse_mode("piscis").unwrap(), HeadlessCliMode::Piscis);
    ...
    fn parse_run_prompt_and_defaults() {
        ...
        assert_eq!(req.mode, HeadlessCliMode::Piscis);
    }
    ...
    #[test]
    fn parse_capabilities_defaults_to_piscis() {
        let mode = parse_capabilities_mode(&[]).unwrap();
        assert_eq!(mode, HeadlessCliMode::Piscis);
    }
