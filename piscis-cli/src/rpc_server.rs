...
//!
//! `openpiscis-headless rpc` spawns this loop to let a parent
//! (`SubprocessSubagentRuntime` in `piscis-kernel::pool::subagent`) drive
//! Koi turns over stdin / stdout. See `piscis-kernel::pool::subagent`'s
//! module docs for the wire format (newline-delimited JSON-RPC 2.0).
//!
//! Scope of this first implementation:
//!
//! * `koi.turn`  — bridge into `piscis_kernel::headless::run_piscis_turn`
...
use crate::runner::run_piscis_once_with_stderr_events;

/// Entry point for the `openpiscis-headless rpc` subcommand. Blocks on
...
fn handle_koi_turn(params: Value, cancel: Arc<AtomicBool>) -> Result<KoiTurnOutcome, String> {
    ...
    // Map the Koi turn onto a piscis-mode headless request. This is the
    // MVP bridge: the child reuses `run_piscis_turn` so we immediately get
    // event streaming + the full neutral tool set. Richer semantics
    // (bespoke Koi system prompt, org-spec injection, etc.) will come
    // once `piscis_kernel::pool::coordinator::execute_todo_turn` is fully
    ...
    // per-iteration cancel gets wired once run_piscis_turn takes an
    ...
    match run_piscis_once_with_stderr_events(cli_request) {
