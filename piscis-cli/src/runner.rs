//! Shared helpers for running a single piscis-mode agent turn, and
//! bootstrapping a pool-mode parent, from any headless host.
//!
//! `openpiscis-headless` dispatches into [`run_piscis_once`] for piscis-mode
...
//! `openpiscis-headless run --mode pool`: it creates (or attaches to) a
...
//! as subprocesses via [`piscis_kernel::pool::SubprocessSubagentRuntime`].
...
use piscis_kernel::agent::tool::{new_tool_registry_handle, ToolRegistryHandleExt};
use piscis_kernel::headless::{self, HeadlessDeps};
use piscis_kernel::pool::coordinator::CoordinatorConfig;
use piscis_kernel::pool::model::{CallerContext, CreatePoolArgs, SendPoolMessageArgs};
use piscis_kernel::pool::services as pool_services;
use piscis_kernel::pool::store::PoolStore;
use piscis_kernel::pool::SubprocessSubagentRuntime;
...
/// `OPENPISCIS_CONFIG_DIR` env var, then the OS default (`dirs::data_dir`).
pub fn resolve_app_data_dir(request: Option<&HeadlessCliRequest>) -> PathBuf {
    ...
    std::env::var("OPENPISCIS_CONFIG_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|_| {
            dirs::data_dir()
...
/// Execute a single piscis-mode turn end-to-end.
///
/// This is the canonical `piscis-once` pipeline:
///
/// 1. Reject pool mode (desktop-only).
/// 2. Open kernel state (`piscis.db` + `config.json`) under
...
/// 5. Drive [`piscis_kernel::headless::run_piscis_turn`] on a fresh
...
pub fn run_piscis_once(request: HeadlessCliRequest) -> Result<HeadlessCliResponse, String> {
    ...
    run_piscis_once_with_host(request, app_data_dir, host)
}

/// Same as [`run_piscis_once`], but routes progress events away from stdout.
///
/// JSON-RPC subprocess mode owns stdout for protocol frames, so child-side
/// agent/pool events must never be written there.
pub fn run_piscis_once_with_stderr_events(
...
fn run_piscis_once_with_host(
    ...
    // Headless piscis turns participate in pool flows too (a Koi turn
    ...
    //   • `subagent_runtime`            → left `None` for the Piscis
    //     headless path itself; mention fan-out inside a Piscis turn
    ...
    piscis_kernel::tools::register_neutral_tools(&mut handle, &neutral_cfg);
    ...
    runtime
        .block_on(headless::run_piscis_turn(request, deps))
        .map_err(|e| format!("run_piscis_turn failed: {e}"))
}

/// Resolve the `openpiscis-headless` binary this process is currently
/// running as. The subprocess subagent runtime needs this path to spawn
/// Koi children via `openpiscis-headless rpc`.
///
/// Lookup order: `PISCIS_HEADLESS_BIN` env var → `current_exe()` → fall
/// back to the bare name and let the OS resolve it via `PATH`.
fn resolve_headless_binary() -> PathBuf {
    if let Ok(raw) = std::env::var("PISCIS_HEADLESS_BIN") {
        let raw = raw.trim();
        if !raw.is_empty() {
    ...
    if cfg!(windows) {
        PathBuf::from("openpiscis-headless.exe")
    } else {
        PathBuf::from("openpiscis-headless")
    }
}

/// Minimum-viable pool-mode parent runner.
///
/// This is the `openpiscis-headless run --mode pool` entry point. It
...
///   * Koi turns run as `openpiscis-headless rpc` subprocesses via
...
fn render_pool_wait_summary(summary: &PoolWaitSummary) -> String {
    ...
    if summary.requires_supervisor_closeout {
        return format!(
            "Koi work finished: {} todo(s) done. Closeout status: {}. Piscis supervisor must now review pool messages and explicitly merge or request rework.",
