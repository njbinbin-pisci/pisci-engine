//! Interactive REPL for `openpiscis-headless`.
//!
//! Entered via `openpiscis-headless chat` or by running the binary with no
//! arguments. Drives a multi-turn piscis-mode conversation against the
//! local kernel state (same `piscis.db` + `config.json` the desktop app
...
//!     should keep using `openpiscis-headless run`.
...
impl PoolEventSink for InteractiveEventSink {
    fn emit_pool(&self, _event: &PoolEvent) {
        // Interactive mode is single-agent piscis only — pool events are
...
impl ReplState {
    fn new() -> Self {
        Self {
            session_id: None,
            workspace: None,
            verbose_tools: std::env::var("OPENPISCIS_VERBOSE_TOOLS")
...
pub fn run_interactive() -> Result<(), String> {
    ...
    loop {
        ...
        // Normal prompt → run one piscis turn.
...
async fn run_one_turn(state: &mut ReplState, prompt: &str) -> Result<(), String> {
    ...
    let response = headless::run_piscis_turn(request, deps)
        .await
        .map_err(|e| format!("{e}"))?;
