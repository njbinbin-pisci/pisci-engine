//! Confirms that the `openpiscis-headless` tool surface includes the pool
//! and plan tools.
//!
...
use piscis_cli::host::CliHost;
use piscis_core::host::{HostRuntime, ToolRegistryHandle};
use piscis_kernel::agent::plan::new_plan_store;
use piscis_kernel::agent::tool::{new_tool_registry_handle, ToolRegistryHandleExt};
use piscis_kernel::store::{Database, Settings};
use piscis_kernel::tools::{register_neutral_tools, NeutralToolsConfig};
...
fn scratch_dir() -> PathBuf {
    let nanos = std::time::SystemTime::now()
        ...
        .map(|d| d.as_nanos())
        .unwrap_or(0);
    let dir = std::env::temp_dir().join(format!("piscis-cli-pool-tools-{nanos}"));
