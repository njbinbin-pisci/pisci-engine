//! End-to-end integration tests for the `openpiscis-headless` binary.
//!
//! These tests launch the compiled binary via Cargo's
//! `CARGO_BIN_EXE_openpiscis-headless` hook and exercise the public CLI
...
//! (gated on `OPENPISCIS_TEST_API_KEY`) covers the full end-to-end turn.

// Integration tests spawn the piscis-cli binary directly; raw Command::new is appropriate here.
#![allow(clippy::disallowed_methods)]
use std::path::PathBuf;
use std::process::Command;

fn bin_path() -> PathBuf {
    PathBuf::from(env!("CARGO_BIN_EXE_openpiscis-headless"))
}
...
fn run(args: &[&str]) -> (std::process::Output, String, String) {
    let output = Command::new(bin_path())
        .args(args)
        .output()
        .expect("failed to execute openpiscis-headless binary");
...
fn version_prints_kernel_version() {
    ...
    assert!(
        stdout.contains("openpiscis-headless"),
...
fn capabilities_emits_expected_schema() {
    ...
    // Pool / plan tools live in piscis-kernel and must NOT be flagged as
    // disabled for the CLI host.
    for newly_enabled in ["plan_todo", "pool_org", "pool_chat"] {
        assert!(
            !names.contains(&newly_enabled),
            "{newly_enabled} should be enabled for openpiscis-headless, still listed in {names:?}"
        );
    }
}
...
fn run_smoke_reports_missing_api_key_by_default() {
    ...
    let out = Command::new(bin_path())
        .env_remove("OPENPISCIS_TEST_API_KEY")
        .env("OPENPISCIS_CONFIG_DIR", &cfg)
        .args([
            "run",
            "--prompt",
            "quick ping",
...
// state → HeadlessDeps → run_piscis_turn → event sink → response JSON).
//
// It is **gated** behind `OPENPISCIS_TEST_API_KEY` because:
...
//
// Optional knobs:
//   * `OPENPISCIS_TEST_PROVIDER` (default `anthropic`)
//   * `OPENPISCIS_TEST_MODEL`    (default `claude-haiku-4-5`)
//   * `OPENPISCIS_TEST_BASE_URL` (default empty; lets users point at an
//     internal proxy / Bedrock-compatible endpoint)
//   * `OPENPISCIS_TEST_PROMPT`   (default `What is 2+2? Reply with just the number.`)
//   * `OPENPISCIS_TEST_EXPECT`   (default `4`)
...
fn e2e_run_returns_answer_with_real_api_key() {
    let api_key = match std::env::var("OPENPISCIS_TEST_API_KEY") {
        Ok(key) if !key.trim().is_empty() => key,
        _ => {
            eprintln!(
                "skipping e2e_run_returns_answer_with_real_api_key — set \
                 OPENPISCIS_TEST_API_KEY to enable (and optionally \
                 OPENPISCIS_TEST_PROVIDER/MODEL/BASE_URL)."
            );
            return;
        }
    };

    let provider = std::env::var("OPENPISCIS_TEST_PROVIDER")
        .ok()
        .map(|s| s.trim().to_string())
        .filter(|s| !s.is_empty())
        .unwrap_or_else(|| "anthropic".to_string());
    let model = std::env::var("OPENPISCIS_TEST_MODEL")
        .ok()
        .map(|s| s.trim().to_string())
        .filter(|s| !s.is_empty())
        .unwrap_or_else(|| "claude-haiku-4-5".to_string());
    let base_url = std::env::var("OPENPISCIS_TEST_BASE_URL").unwrap_or_default();
    let prompt = std::env::var("OPENPISCIS_TEST_PROMPT")
        .unwrap_or_else(|_| "What is 2+2? Reply with just the number.".to_string());
    let expect = std::env::var("OPENPISCIS_TEST_EXPECT").unwrap_or_else(|_| "4".to_string());
    ...
    assert!(
        out.status.success(),
        "openpiscis-headless run failed\nstderr={stderr}"
    );
    ...
    assert_eq!(response["mode"], "piscis");
...
mod tempdir_impl {
    ...
    pub fn tempdir() -> std::io::Result<TempDir> {
        let nanos = std::time::SystemTime::now()
            ...
            .map(|d| d.as_nanos())
            .unwrap_or(0);
        let path = std::env::temp_dir().join(format!("openpiscis-cli-it-{nanos}"));
