...
pub fn discover_project_instruction_files(
    ...
    for dir in dirs {
        for candidate in [
            dir.join("PISCIS.md"),
            dir.join("PISCIS.local.md"),
            dir.join(".piscis").join("PISCIS.md"),
            dir.join(".piscis").join("instructions.md"),
        ] {
...
mod tests {
    ...
    fn temp_dir() -> PathBuf {
        let nanos = SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir().join(format!("piscis-project-context-{nanos}"))
    }
    ...
    fn discovers_instruction_files_from_ancestor_chain() {
        ...
        fs::create_dir_all(root.join(".piscis")).expect("root .piscis");
        fs::create_dir_all(nested.join(".piscis")).expect("nested .piscis");
        fs::write(root.join("PISCIS.md"), "root rules").expect("root rules");
        fs::write(root.join(".piscis").join("instructions.md"), "shared rules")
            .expect("shared rules");
        fs::write(
            nested.join(".piscis").join("instructions.md"),
            "local rules",
        )
        .expect("local rules");
    ...
    fn dedupes_equivalent_instruction_content() {
        ...
        fs::write(root.join("PISCIS.md"), "same rules\n\n").expect("root write");
        fs::write(nested.join("PISCIS.md"), "same rules\n").expect("nested write");
    ...
    fn renders_budgeted_instruction_context() {
        ...
        fs::write(root.join("PISCIS.md"), "x".repeat(5000)).expect("write rules");
