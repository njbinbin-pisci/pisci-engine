...
//!
//! These tools depend only on `piscis-kernel` and `piscis-core`, and do not
...
use piscis_core::host::{EventSink, PoolEventSink, SubagentRuntime, ToolRegistryHandle};
...
pub struct NeutralToolsConfig {
    ...
    /// Sink for [`piscis_core::host::PoolEvent`] — required to enable
    ...
    /// run Kois for delegated `@!mention`, `assign_koi`,
...
///
/// Hosts typically call this from their [`piscis_core::host::HostTools::register`]
