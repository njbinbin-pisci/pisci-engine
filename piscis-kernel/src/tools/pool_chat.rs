...
//! (one per Koi + one for Piscis) which is no longer necessary.
...
use piscis_core::models::{KoiDefinition, PoolMessage};
...
impl Tool for PoolChatTool {
    ...
    fn description(&self) -> &str {
        "Communicate in the project pool chat with your team members. \
         \
         Actions: \
         - 'send': Post a message to pool chat as yourself. Use plain `@KoiName` / `@all` for notification only; it never wakes Koi execution. Use `@!KoiName` / `@!all` only when you are explicitly delegating concrete work that should create or wake active execution; live delegated mentions must be at the start of the message or the start of their own line. When the project needs explicit coordination, include a concrete handoff and a `[ProjectStatus] ...` signal. Koi completion is not final delivery until Piscis supervisor reviews and merges or requests rework. \
    ...
    async fn call(&self, input: Value, ctx: &ToolContext) -> anyhow::Result<ToolResult> {
        ...
        match action {
            "send" => {
                if caller.is_piscis() {
                    return Ok(ToolResult::err(
                        "Piscis cannot send pool_chat messages directly. Use pool_org actions such as assign_koi, post_status, or get_messages instead.",
                    ));
                }
            ...
            "reply" => {
                if caller.is_piscis() {
                    return Ok(ToolResult::err(
                        "Piscis cannot reply in pool_chat directly. Use pool_org actions such as post_status or get_messages instead.",
                    ));
                }
                let content = input["content"].as_str().unwrap_or("");
                if content.trim().is_empty() {
