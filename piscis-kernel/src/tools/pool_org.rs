...
use crate::pool::model::{
    AssignKoiArgs, CallerContext, CreatePoolArgs, CreateTodoArgs, DeleteTodoArgs,
    MergeBranchesArgs, PostStatusArgs, ReplaceTodoArgs, UpdateOrgSpecArgs, UpdateTodoStatusArgs,
    WaitForKoiArgs,
};
use crate::pool::{services, PoolStore};
use async_trait::async_trait;
use piscis_core::host::{PoolEventSink, SubagentRuntime};
...
impl PoolOrgTool {
    ...
    /// `pool_sessions.origin_im_binding_key` when an IM-driven Piscis
...
impl Tool for PoolOrgTool {
    ...
    fn description(&self) -> &str {
        "Manage project pools, lifecycle, and organization specs. Use this to set up collaborative projects. \
         ...
         - 'post_status': Piscis-only controlled status message for supervisor notes, decisions, or waiting explanations. This does not trigger @! mention fan-out. \
         - 'wait_for_koi': Piscis-only real elapsed-time wait after assigning/resuming/replacing work. Uses host-side sleep/backoff and returns structured status counts. \
         ...
         - 'complete_todo': Mark a todo as done (requires todo_id, summary). The summary is a concise description of what was accomplished — it becomes the visible result in the pool chat. Piscis can complete any todo; Koi can only complete their own. Completing a todo does NOT hand off the next step automatically. \
         - 'cancel_todo': Cancel a todo (requires todo_id, optional reason). Piscis can cancel any todo; Koi can only cancel their own — to cancel someone else's, @piscis in pool_chat. \
         - 'resume_todo': Resume a blocked or needs_review todo (requires todo_id). This restarts execution for the existing task from its current project context. Piscis should decide when to use this. \
         - 'replace_todo': Replace an existing todo with a new owner/task (requires todo_id, new_owner_id, task, reason). This cancels the original todo so it cannot be resumed, creates a replacement todo, and notifies the new owner. Piscis should decide when to use this. \
         - 'delete_todo': Permanently delete todo rows from the board. Use `todo_id` for a single delete, or `pool_id` plus `delete_status` / `delete_owner_id` for filtered batch cleanup (for example deleting all cancelled todos in one pool). Piscis-only. \
         - 'update_todo_status': Update a todo's status (requires todo_id, status). Piscis can change any; Koi can only change their own. Valid statuses: todo, in_progress, blocked. This changes task-board state, but teammates still need an explicit `pool_chat` update if they should react. \
         - 'merge_branches': Piscis-only supervisor integration action. Merge one Koi branch (pass branch=...) or all remaining koi/* branches into main after review. Prefer incremental single-branch merges when integration_ready todos exist on the board. \
         \
    ...
    fn input_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                ...
                "event_type": {
                    "type": "string",
                    "description": "For post_status: optional structured event label (default piscis_status)"
                },
                "name": {
                    "type": "string",
...
fn render_list_pools(v: &Value) -> String {
    use piscis_core::models::{KoiDefinition, PoolSession};
    let raw_sessions = v
        .get("raw_sessions")
...
fn render_find_related(v: &Value, keywords: &str) -> String {
    use piscis_core::models::PoolSession;
    let arr = v
        .get("sessions")
        .and_then(|s| s.as_array())
        .cloned()
...
fn render_messages(v: &Value) -> String {
    use piscis_core::models::{PoolMessage, PoolSession};
