...
pub async fn execute_todo_turn(
    ...
    if !piscis_core::integration::is_todo_dependency_satisfied(&todo, &todos) {
        anyhow::bail!(
...
pub async fn handle_mention(
    ...
    // ── @!Piscis: Handle Piscis self-targeting ──────────────────────────
    // Piscis is NOT a Koi; it does not appear in the Koi list. But users
    // expect @!Piscis to work just like any other delegated mention.
    if sender_id != "piscis" && has_live_delegated_mention(content, "Piscis") {
        ...
        let todo = store
            .write(move |db| {
                ...
                // FOREIGN KEY constraint. "piscis" is a synthetic owner that
                ...
                if let Err(e) = db.upsert_koi_with_id("piscis", "Piscis") {
                    tracing::warn!(
                        target: "pool::coordinator",
                        "@!Piscis: failed to upsert sentinel kois row: {e}"
                    );
                }
                db.create_koi_todo(
                    "piscis",
        ...
        tracing::info!(
            ...
            todo_id = %todo.id,
            "@!Piscis mention recorded as board todo (Piscis handles it in main loop)"
        );
    }
...
fn has_live_delegated_mention(content: &str, name: &str) -> bool {
    ...
    content.lines().any(|line| {
        ...
        // case-insensitive prefix match: @!piscis and @!Piscis both work
...
mod tests {
    use super::*;
    use piscis_core::host::NullPoolEventSink;
    ...
    fn source_type_buckets() {
        assert_eq!(todo_source_type("piscis"), "piscis");
    ...
    fn plain_mentions_do_not_dispatch_koi_turns() {
        ...
        assert!(parse_mention_targets(&kois, "piscis", "@all 请同步一下").is_empty());
        assert!(parse_mention_targets(&kois, "piscis", "@Alpha 看一下").is_empty());
    }

    #[test]
    fn piscis_delegated_mention_is_detected() {
        let kois: Vec<KoiDefinition> = vec![
            test_koi("a", "Alpha", "idle"),
            test_koi("b", "Beta", "idle"),
        ];
        // @!Piscis should NOT be returned by parse_mention_targets
        // (Piscis is not a Koi; it is handled separately in handle_mention).
        let targets = parse_mention_targets(&kois, "user", "@!Piscis 帮我做报告");
        assert!(
            targets.is_empty(),
            "@!Piscis should not appear in Koi-iteration targets"
        );
        // But has_live_delegated_mention detects it directly.
        assert!(has_live_delegated_mention("@!Piscis 帮我做报告", "Piscis"));
        assert!(has_live_delegated_mention("@!piscis do task", "Piscis"));
        assert!(!has_live_delegated_mention("@Piscis chat", "Piscis"));
    }
    ...
    fn delegated_mentions_dispatch_only_targeted_kois() {
        ...
        let one = parse_mention_targets(&kois, "piscis", "@!Alpha 请实现");
        assert_eq!(one.len(), 1);
        assert_eq!(one[0].koi.id, "a");

        let all = parse_mention_targets(&kois, "piscis", "@!all 请分工");
    ...
    fn delegated_mentions_only_dispatch_when_live_at_line_start() {
        ...
        let multiline =
            parse_mention_targets(&kois, "piscis", "Context first\n  @!Beta: review it");
