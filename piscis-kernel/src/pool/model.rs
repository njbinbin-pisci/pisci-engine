...
///
/// `memory_owner_id` is either `"piscis"` (the coordinator) or the Koi's
/// id when a Koi-scene turn invoked the tool. Services enforce the
/// "piscis may do anything / koi can only manage its own todos" rule
/// from this field.
#[derive(Clone)]
...
impl<'a> CallerContext<'a> {
    pub fn is_piscis(&self) -> bool {
        self.memory_owner_id == "piscis"
    }
...
pub struct CreatePoolArgs {
    ...
    /// originated this pool — set when an IM-driven Piscis session
