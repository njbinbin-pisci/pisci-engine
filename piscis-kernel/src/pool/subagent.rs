...
//! # Wire protocol (v1)
//!
//! The parent (desktop / CLI host) drives a child `openpiscis-headless`
...
//!
//! * `event.agent` — a serialised `piscis_core::agent::AgentEvent` (tool
//!   calls, streaming text …). Parents that surface per-event UI route
//!   these straight into their `EventSink`.
//! * `event.pool` — a serialised [`piscis_core::host::PoolEvent`] (the
...
/// Spawns `openpiscis-headless rpc` as a child process per turn and
...
pub struct SubprocessSubagentRuntime {
    ...
    /// Extra environment variables to pass through (e.g. `OPENPISCIS_LLM_*`).
...
impl SubprocessSubagentRuntime {
    ...
    fn build_command(&self) -> Command {
        ...
        if let Some(dir) = &self.app_data_dir {
            cmd.env("OPENPISCIS_CONFIG_DIR", dir);
        }
...
impl SubagentRuntime for SubprocessSubagentRuntime {
    async fn spawn_koi_turn(&self, request: KoiTurnRequest) -> anyhow::Result<KoiTurnHandle> {
        ...
        let mut child = cmd
            .spawn()
            .map_err(|e| anyhow::anyhow!("failed to spawn openpiscis-headless: {e}"))?;

        let stdin = child
            .stdin
            .take()
            .ok_or_else(|| anyhow::anyhow!("openpiscis-headless stdin unavailable after spawn"))?;
        let stdout = child
            .stdout
            .take()
            .ok_or_else(|| anyhow::anyhow!("openpiscis-headless stdout unavailable after spawn"))?;
        let stderr = child
            .stderr
            .take()
            .ok_or_else(|| anyhow::anyhow!("openpiscis-headless stderr unavailable after spawn"))?;
...
mod tests {
    use super::*;
    use piscis_core::host::KoiTurnRequest;
