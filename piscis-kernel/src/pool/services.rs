...
//! [`piscis_core::host::SubagentRuntime`]).
...
use piscis_core::host::{PoolEvent, PoolEventSink, SubagentRuntime, TodoChangeAction};
...
fn check_todo_ownership(todo: &KoiTodo, caller: &CallerContext<'_>) -> anyhow::Result<()> {
    if caller.is_piscis() || todo.owner_id == caller.memory_owner_id {
        return Ok(());
    }
    anyhow::bail!(
        "Permission denied. You can only manage your own todos. This todo belongs to '{}'. \
         To cancel or modify another agent's task, @piscis in pool_chat to request it.",
        todo.owner_id
    )
}

fn check_piscis_only(caller: &CallerContext<'_>, action: &str) -> anyhow::Result<()> {
    if caller.is_piscis() {
        return Ok(());
    }
    anyhow::bail!(
        "Permission denied. Action '{}' is restricted to piscis because it permanently deletes shared board history.",
        action
    )
}
...
pub async fn create_pool(
    ...
    Ok(json!({
        "pool": piscis_core::host::PoolSessionSnapshot::from(&session),
        "git_info": git_info,
        "summary": format!(
...
pub async fn update_org_spec(
    ...
    Ok(json!({
        "pool": piscis_core::host::PoolSessionSnapshot::from(&fresh),
...
pub async fn list_pools(store: &PoolStore) -> anyhow::Result<Value> {
    ...
    Ok(json!({
        "sessions": sessions
            .iter()
            .map(piscis_core::host::PoolSessionSnapshot::from)
            .collect::<Vec<_>>(),
...
pub async fn set_pool_status(
    ...
    let snap: piscis_core::host::PoolSessionSnapshot = (&fresh).into();
...
/// signals + `@piscis` mentions, inserts into DB, emits
...
pub async fn post_status(
    ...
    if !caller.is_piscis() {
        anyhow::bail!("Only Piscis may publish supervisor status through post_status.");
    }
    ...
    Ok(json!({
        "pool_id": session.id,
        "message": msg,
        "summary": "Piscis status posted through pool_org without mention fan-out.",
    }))
}
...
pub async fn wait_for_koi(
    ...
    if !caller.is_piscis() {
        anyhow::bail!("Only Piscis may wait on Koi execution through wait_for_koi.");
    }
...
pub async fn assign_koi(
    ...
    let todo = {
        ...
        let assigned_by = "piscis".to_string();
        let pool_id = session.id.clone();
    ...
    let msg = store
        .write(move |db| {
            db.insert_pool_message(&pool_id, "piscis", &mention_clone, "mention", &meta_str)
        })
        .await?;
    emit_message(sink.as_ref(), &msg);
    ...
    if let Some(sub) = subagent {
        ...
        tokio::spawn(async move {
            let todos = store
                ...
                .await
                .unwrap_or_default();
            if !piscis_core::integration::is_todo_dependency_satisfied(&todo_for_dep, &todos) {
                tracing::info!(
                    target: "pool::services",
    ...
    Ok(json!({
        ...
        "koi_id": koi_id,
        "koi_name": koi_name,
        "todo": piscis_core::host::TodoSnapshot::from(&todo),
        "mention_message": msg,
...
pub async fn claim_todo(
    ...
    Ok(json!({
        "todo": piscis_core::host::TodoSnapshot::from(&refreshed),
        "summary": format!(
...
pub async fn complete_todo(
    ...
    // This matters when Piscis completes a Koi's todo from outside the
...
pub async fn delete_todo(
    ...
    check_piscis_only(caller, "delete_todo")?;
    ...
    if let Some(todo_id) = todo_id {
        ...
        return Ok(json!({
            "deleted_count": 1,
            "deleted_todos": [piscis_core::host::TodoSnapshot::from(&todo)],
            "summary": format!(
                "Deleted todo '{}' ({}).",
                short(&todo.id),
                todo.title
    ...
    Ok(json!({
        ...
        "deleted_todos": todos_to_delete
            .iter()
            .map(piscis_core::host::TodoSnapshot::from)
            .collect::<Vec<_>>(),
        "summary": format!(
...
pub async fn resume_todo(
    ...
    if !caller.is_piscis() {
        anyhow::bail!("Only Piscis may decide whether a blocked task should be resumed.");
    }
    let todo = find_todo_by_prefix(store, todo_id)
        .await?
        .ok_or_else(|| anyhow::anyhow!("Todo '{}' not found", todo_id))?;
...
pub async fn replace_todo(
    ...
    if !caller.is_piscis() {
        anyhow::bail!("Only Piscis may replace a task owner.");
    }
    ...
    Ok(json!({
        "original": piscis_core::host::TodoSnapshot::from(&original),
        "replacement": piscis_core::host::TodoSnapshot::from(&replacement),
        "summary": format!(
...
pub async fn merge_branches(
    ...
    for result in &results {
        let status = match &result.outcome {
            MergeOutcome::Merged => piscis_core::integration::INTEGRATION_MERGED,
            MergeOutcome::Conflict { .. } | MergeOutcome::Error { .. } => {
                piscis_core::integration::INTEGRATION_CONFLICT
            }
        };
        let pool_id = session.id.clone();
