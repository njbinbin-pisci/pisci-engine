...
pub struct ToolContext {
    ...
    /// Memory owner: "piscis" for the main agent, or a koi_id for Koi agents.
...
//
// `piscis-core` keeps the handle type-erased (no kernel dependency). Hosts
...
//
// ```ignore
// use piscis_core::host::{HostTools, ToolRegistryHandle};
// use piscis_kernel::agent::tool::{Tool, ToolRegistryHandleExt};
//
...
pub use piscis_core::host::ToolRegistryHandle;

/// Build a fresh [`ToolRegistryHandle`] wrapping an empty [`ToolRegistry`].
/// Hosts rarely need this directly; `piscis-kernel` uses it when spawning an
