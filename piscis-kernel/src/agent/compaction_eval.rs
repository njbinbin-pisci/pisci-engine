...
//! Enable with:
//! ```text
//! $env:PISCIS_EVAL_DB = "C:\path\to\piscis-copy.db"
//! $env:PISCIS_EVAL_TOP_N = "5"            # optional, default 5
//! $env:PISCIS_EVAL_MIN_MESSAGES = "40"    # optional, default 40
//! $env:PISCIS_EVAL_MIN_REDUCTION_PCT = "30"
...
#[test]
#[ignore = "reads a real DB from $PISCIS_EVAL_DB; run manually with --ignored --nocapture"]
fn eval_real_sessions() {
    let db_path = match std::env::var("PISCIS_EVAL_DB") {
        Ok(p) => PathBuf::from(p),
        Err(_) => {
            eprintln!("PISCIS_EVAL_DB not set — skipping");
            return;
        }
    };
    let top_n = env_usize("PISCIS_EVAL_TOP_N", 5);
    let min_messages = env_usize("PISCIS_EVAL_MIN_MESSAGES", 40);
    ...
    // p9 — optional acceptance gate. When `PISCIS_EVAL_MIN_REDUCTION_PCT`
    ...
    if let Ok(raw) = std::env::var("PISCIS_EVAL_MIN_REDUCTION_PCT") {
        let min_pct: f64 = raw
            .parse()
            .expect("PISCIS_EVAL_MIN_REDUCTION_PCT must be a number, e.g. \"30\" for 30%");
        assert!(
            agg_full > 0,
