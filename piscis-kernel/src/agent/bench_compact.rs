...
//!
//! Exposes Piscis's two compaction tiers as a simple JSON-in/JSON-out API
...
//!    One LLM call.
//!
//! Used by `src-tauri/piscis-cli/src/bin/piscis_compact_one.rs` for the cross-framework
...
pub async fn compact_one(req: BenchRequest) -> Result<BenchResponse> {
    ...
    let (compressed_msgs, llm_calls, llm_in, llm_out) = match mode.as_str() {
        ...
        "HARNESS" => {
            ...
            // path uses, so the benchmark measures Piscis's actual
            // production context assembly.
            let tool_minimals = collect_tool_minimals(&req.messages);
            let lp =
                crate::agent::harness::LayeredPrompt::from_monolithic("You are a Piscis agent.");
